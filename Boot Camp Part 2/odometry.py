...
class Odometry(Node):
    ...
    def encoder_callback(self, msg: Encoders):
        ...
        Your task is to update the odometry based on the encoder data in 'msg'. 
        ...
        angular_velocity_left = delta_ticks_left / (ticks_per_rev * dt) * 2 * math.pi
        angular_velocity_right = delta_ticks_right / (ticks_per_rev * dt) * 2 * math.pi

        v = wheel_radius / 2 * (angular_velocity_right + angular_velocity_left)
        w = wheel_radius / base * (angular_velocity_right - angular_velocity_left)
        ...
        self.transform_and_broadcast(stamp, self._x, self._y, self._yaw)
        
        self.publish_path(stamp, self._x, self._y, self._yaw)


    def transform_and_broadcast(self, stamp, x, y, yaw):
